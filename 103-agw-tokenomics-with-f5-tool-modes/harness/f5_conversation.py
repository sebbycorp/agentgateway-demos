"""Multi-turn F5 conversation: ask 5 questions in ONE ongoing chat per tool mode,
and show how token cost COMPOUNDS across the conversation — plus real cache reads.

Why this matters: in a real conversation the gateway re-sends the tool definitions
on every turn. Standard re-pays the full 29-tool F5 catalog each turn; Search sends
2 meta-tools each turn. Over 5 turns that gap compounds. gpt-5.5 also caches the
stable prefix, so we capture cache_read tokens and price them at the cached rate.

Prereqs: F5 backends + proxy:8080 + pushgateway:9091. Backend gpt-5.5 → the script
omits temperature automatically.
"""
import asyncio
import json
import os
...
CONVERSATION = [
    "How many LTM pools are in the Common partition?",
    "Now list the virtual servers and their destinations.",
    "What BIG-IP version and platform is this, and what's the failover status?",
    "Which SSL certificates are expiring, with their dates?",
    "Summarize everything you found about this F5 in a short status report.",
]
...
async def main():
    ...
    labels = ["mode", "turn"]
    g_prompt = Gauge("agw_f5conv_cum_prompt_tokens", "cumulative prompt tokens", labels, registry=reg)
    g_total = Gauge("agw_f5conv_cum_total_tokens", "cumulative total tokens", labels, registry=reg)
    g_cached = Gauge("agw_f5conv_cum_cached_tokens", "cumulative cache-read tokens", labels, registry=reg)
    g_cost = Gauge("agw_f5conv_cum_cost", "cumulative cache-aware USD", labels, registry=reg)
    ...
    try:
        delete_from_gateway(PUSHGATEWAY, job="agw_f5_conversation")
    except Exception:
        pass
    try:
        push_to_gateway(PUSHGATEWAY, job="agw_f5_conversation", registry=reg)
