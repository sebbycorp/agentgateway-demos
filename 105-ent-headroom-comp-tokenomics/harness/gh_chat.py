...
# Headroom knob: set LLM_URL to the Headroom proxy (HEADROOM=on path) to route the
# LLM call through compression; defaults to AGW /openai (Headroom OFF).
    ...
    async with httpx.AsyncClient(timeout=120) as client:
        async with streamablehttp_client(GW + route) as (r, w, _):
            async with ClientSession(r, w) as session:
                ...
                print(f"\n=== GitHub via {mode.upper()} mode ({route}) | Headroom {hr} | llm={LLM} ===")
