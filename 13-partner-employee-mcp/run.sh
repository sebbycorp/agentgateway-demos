...
if ! command -v npx >/dev/null 2>&1; then
  echo "ERROR: npx is not on PATH (required for the MCP everything target)." >&2
  exit 1
fi

echo "Starting agentgateway (MCP http://localhost:3000/mcp  LLM http://localhost:4000  admin http://localhost:15000/ui/)"
